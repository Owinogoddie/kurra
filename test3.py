...
# ============================================
SHOW_REALTIME_FRAMES = False  # Set to False to disable real-time display for max speed
VIDEO_PATH = '20.mp4'  # Update this path for your Jetson Nano
OUTPUT_PATH = 'videos/output_video_bytetrack.mp4'  # Output path
MODEL_PATH = 'yolo11n.pt'  # YOLO nano model (faster than yolo11s.pt)

# Performance settings
TARGET_WIDTH = 640
TARGET_HEIGHT = 360
INFERENCE_SIZE = 416  # Smaller inference size for faster processing
SKIP_FRAMES = 1  # Process every frame (set to 2 or 3 to skip frames for more speed)
CONF_THRESHOLD = 0.5
IOU_THRESHOLD = 0.7
...
# NEW LINE COORDINATES FOR 640x360 RESOLUTION
# Out of town line: from (138, 129) to (269, 121)
OUT_LINE_X1, OUT_LINE_Y1 = 138, 129
OUT_LINE_X2, OUT_LINE_Y2 = 269, 121

# To town line: from (276, 134) to (551, 125)
TO_TOWN_LINE_X1, TO_TOWN_LINE_Y1 = 276, 134
TO_TOWN_LINE_X2, TO_TOWN_LINE_Y2 = 551, 125

print("Starting video processing with YOLO 11 Nano + ByteTrack...")
print(f"Out of town line: ({OUT_LINE_X1},{OUT_LINE_Y1}) to ({OUT_LINE_X2},{OUT_LINE_Y2})")
print(f"To town line: ({TO_TOWN_LINE_X1},{TO_TOWN_LINE_Y1}) to ({TO_TOWN_LINE_X2},{TO_TOWN_LINE_Y2})")

# Function to check if point crosses a line (improved for diagonal lines)
def point_crosses_line(x, y, x1, y1, x2, y2, tolerance=15):
    """
...
while True:
    ...
    # Process detection based on frame skip setting
    if frame_skip_counter >= SKIP_FRAMES:
        frame_skip_counter = 0
        
        # YOLO 11 prediction with ByteTrack tracking - OPTIMIZED
        results = model.track(frame,
                             persist=True,              # Keep track IDs consistent across frames
                             tracker="bytetrack.yaml",  # Use ByteTrack algorithm
                             conf=CONF_THRESHOLD,       # Detection confidence threshold
                             iou=IOU_THRESHOLD,         # IoU threshold for NMS
                             imgsz=INFERENCE_SIZE,      # Inference size for faster processing
                             classes=[2, 3, 5, 7],      # Only detect: car, motorcycle, bus, truck
                             verbose=False,
                             half=True)                 # Use FP16 for speed
        
        last_results = results
    else:
        # Use last results if skipping frames
        results = last_results if last_results is not None else model.track(frame, persist=True, tracker="bytetrack.yaml", conf=CONF_THRESHOLD, verbose=False)
    ...
    if results and len(results[0].boxes) > 0 and results[0].boxes.id is not None:
        ...
        for i in range(len(boxes)):
            ...
            if any(vehicle in class_name for vehicle in vehicle_types):
                ...
                # Check for crossing OUT OF TOWN line (red line)
                if point_crosses_line(cx, cy, OUT_LINE_X1, OUT_LINE_Y1, OUT_LINE_X2, OUT_LINE_Y2):
                    if not tracked_vehicles[track_id]['counted_out']:
                        ...
                        cv2.putText(frame, f"OUT-{class_name}-{track_id}", (cx, cy-10),
                                   cv2.FONT_HERSHEY_COMPLEX, 0.4, (0, 0, 255), 1)

                # Check for crossing TO TOWN line (blue line)
                if point_crosses_line(cx, cy, TO_TOWN_LINE_X1, TO_TOWN_LINE_Y1, TO_TOWN_LINE_X2, TO_TOWN_LINE_Y2):
                    if not tracked_vehicles[track_id]['counted_to_town']:
                        ...
                        cv2.putText(frame, f"TO-TOWN-{class_name}-{track_id}", (cx, cy-10),
    ...
    # Draw OUT OF TOWN line (red)
    cv2.line(frame, (OUT_LINE_X1, OUT_LINE_Y1), (OUT_LINE_X2, OUT_LINE_Y2), red_color, 3)
    cv2.putText(frame, 'OUT OF TOWN', (OUT_LINE_X1 - 50, OUT_LINE_Y1 + 20), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.4, red_color, 1)

    # Draw TO TOWN line (blue)
    cv2.line(frame, (TO_TOWN_LINE_X1, TO_TOWN_LINE_Y1), (TO_TOWN_LINE_X2, TO_TOWN_LINE_Y2), blue_color, 3)
    cv2.putText(frame, 'TO TOWN', (TO_TOWN_LINE_X1 + 10, TO_TOWN_LINE_Y1 - 10), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.4, blue_color, 1)

    # Display counters - OUT OF TOWN (left side)
    y_pos = 20
    cv2.putText(frame, '=== OUT OF TOWN ===', (5, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.4, red_color, 1)
    y_pos += 18
    cv2.putText(frame, f'Cars: {len(counter_out_cars)}', (5, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.35, text_color, 1)
    y_pos += 15
    cv2.putText(frame, f'Trucks: {len(counter_out_trucks)}', (5, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.35, text_color, 1)
    y_pos += 15
    cv2.putText(frame, f'Buses: {len(counter_out_buses)}', (5, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.35, text_color, 1)
    y_pos += 15
    cv2.putText(frame, f'Motorcycles: {len(counter_out_motorcycles)}', (5, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.35, text_color, 1)

    # Display counters - TO TOWN (left side, below out of town)
    y_pos += 25
    cv2.putText(frame, '=== TO TOWN ===', (5, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.4, blue_color, 1)
    y_pos += 18
    cv2.putText(frame, f'Cars: {len(counter_to_town_cars)}', (5, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.35, text_color, 1)
    y_pos += 15
    cv2.putText(frame, f'Trucks: {len(counter_to_town_trucks)}', (5, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.35, text_color, 1)
    y_pos += 15
    cv2.putText(frame, f'Buses: {len(counter_to_town_buses)}', (5, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.35, text_color, 1)
    y_pos += 15
    cv2.putText(frame, f'Motorcycles: {len(counter_to_town_motorcycles)}', (5, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.35, text_color, 1)
    ...
    cv2.putText(frame, f'TOTAL OUT: {total_out}', (TARGET_WIDTH-180, 25), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, red_color, 1)
    cv2.putText(frame, f'TOTAL TO TOWN: {total_to_town}', (TARGET_WIDTH-180, 50), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, blue_color, 1)

    # Add frame info with ByteTrack indicator
    cv2.putText(frame, f'Frame: {count}/{total_frames}', (TARGET_WIDTH-180, TARGET_HEIGHT-15),
               cv2.FONT_HERSHEY_SIMPLEX, 0.35, text_color, 1)

    # Show number of currently tracked vehicles
    current_tracked = len(results[0].boxes) if results and len(results[0].boxes) > 0 and results[0].boxes.id is not None else 0
    cv2.putText(frame, f'Tracked: {current_tracked}', (TARGET_WIDTH-180, TARGET_HEIGHT-30),
               cv2.FONT_HERSHEY_SIMPLEX, 0.35, text_color, 1)

    # Add ByteTrack watermark
    cv2.putText(frame, 'YOLO11n+ByteTrack', (TARGET_WIDTH-150, TARGET_HEIGHT-45),
    ...
    if SHOW_REALTIME_FRAMES:
        cv2.imshow('Vehicle Counter - YOLO11 Nano + ByteTrack', frame)
    ...
    if count % 100 == 0:
        current_vehicles = len(results[0].boxes) if results and len(results[0].boxes) > 0 else 0
...
if os.path.exists(output_path):
    ...
    print(f"Tracking Algorithm: ByteTrack (Optimized for Jetson Nano)")
...
print("\nProcessing complete with optimized ByteTrack!")
print("Optimizations applied:")
print("- YOLO11n (nano) model for faster inference")
print(f"- Reduced resolution: {TARGET_WIDTH}x{TARGET_HEIGHT}")
print(f"- Inference size: {INFERENCE_SIZE}")
print(f"- Frame skipping: {SKIP_FRAMES}")
print("- FP16 half precision")
print("- Vehicle class filtering (car, truck, bus, motorcycle only)")